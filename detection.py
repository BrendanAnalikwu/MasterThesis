...
from loss import shear, strain_rate
from visualisation import load_model, propagate
...
def cut_neighbours(img):
    ...
    x, y = (1, 1)
    overlap = 1
    shape = (((X - 2 * overlap) // x), ((Y - 2 * overlap) // y), x + 2 * overlap,
             y + 2 * overlap)  # number of patches, patch_shape
...
def detect_segments(lkf_thin, eps_thres=0.1, max_ind=500):
    ...
    while num_nodetect > 0:
        ...
        if seg.shape[-1] > 1:
            ...
            dx = (seg_active[:, :, -1] - seg_active[np.arange(seg_active.shape[0]), :, -num_points]) / np.stack(
                [num_points - 1, num_points - 1], axis=1) - (seg_append - seg_active[:, :, -1])
        ...
        if (deactivate_segs_muln.size > 0):
            ...
            neigh_deactivate = neigh_deactivate[neigh_deactivate[:, 0] < 254]
            neigh_deactivate = neigh_deactivate[neigh_deactivate[:, 1] < 254]
            neigh_deactivate = neigh_deactivate[neigh_deactivate[:, 0] > -1]
            neigh_deactivate = neigh_deactivate[neigh_deactivate[:, 1] > -1]
            ...
            new_starts_deact_ind = np.rollaxis(np.array(np.where((num_neighbours[neigh_deactivate[:, 0].astype('int'),
                                                                                 neigh_deactivate[:, 1].astype(
                                                                                     'int')] <= 2) &
                                                                 (nodetect[neigh_deactivate[:, 0].astype('int'),
                                                                           neigh_deactivate[:, 1].astype(
                                                                               'int')] == 1))), 1).squeeze()
            if new_starts_deact_ind.size > 0:
                new_starts = np.append(new_starts,
                                       neigh_deactivate[new_starts_deact_ind].reshape((new_starts_deact_ind.size, 2)),
                                       axis=0)
                nodetect[new_starts[:, 0].astype('int'), new_starts[:, 1].astype('int')] = 0
                nodetect_intm[1:-1, 1:-1] = nodetect.copy()
                # if ind<5:
        ...
        if do_plot:
            ...
            for i in range(seg.shape[0]):
                ...
                ax[0].text(seg[i, 1, ~np.isnan(seg[i, 1, :])][-1] + 0.5, seg[i, 0, ~np.isnan(seg[i, 1, :])][-1] + 0.5,
                           '%i' % i, color='w')
            ...
            for i in range(active_detection.size):
                ...
                if ~np.isnan(seg_append[i, 1]):
                    ...
                    ax[0].plot(seg[active_detection[i], 1, -1] + 0.5, seg[active_detection[i], 0, -1] + 0.5, color='r',
                               marker=mark)
            ...
            for i in range(seg.shape[0]):
                ...
                ax[1].text(seg[i, 1, ~np.isnan(seg[i, 1, :])][-1] + 0.5, seg[i, 0, ~np.isnan(seg[i, 1, :])][-1] + 0.5,
                           '%i' % i, color='w')
            for i in range(active_detection.size):
                if np.any(deactivate_segs_end.copy() == i):
                    mark = 'x'
                ...
                if ~np.isnan(seg_append[i, 1]):
                    ...
                    ax[1].plot(seg[active_detection[i], 1, -1] + 0.5, seg[active_detection[i], 0, -1] + 0.5, color='r',
                               marker=mark)
        ...
        new_starts_unique = np.array(
            [i_seg_start for i_seg_start in new_starts_unique if not np.any(seg_head_unique == i_seg_start)],
            dtype='int')
        ...
        if active_detection.size == 0:
            fac_starts = 100
            new_starts = np.append(
                np.rollaxis(np.array(np.where((nansum_neighbours(nodetect_intm) == 3) & (nodetect == 1))), 1)[
                ::fac_starts, :],
                np.rollaxis(np.array(np.where((nansum_neighbours(nodetect_intm) == 3) & (nodetect == 1))), 1)[
                1::fac_starts, :], axis=0)
            ...
            new_starts = np.append(new_starts, np.rollaxis(
                np.array(np.where((nansum_neighbours(nodetect_intm) <= 2) & (nodetect == 1))), 1), axis=0)
...
def compute_prob(seg_I, segs, eps_segI, eps_segs, dis_thres, angle_thres, eps_thres, ellp_fac=1):
    ...
    for i in range(segs_i.shape[0]):
        ...
        # Determine distance
        p_dis[np.arange(p_dis.size)[mask_eps][mask_dis_i][mask_ang][i]] = elliptical_distance(seg_I_i, seg_II_i,
                                                                                              ellp_fac=ellp_fac,
                                                                                              dis_thres=dis_thres) / dis_thres
...
def init_prob_matrix(segs, eps_segs, dis_thres, angle_thres, eps_thres, ellp_fac=1):
    ...
    for i_s in range(num_segs - 1):
        prob_ma[i_s, i_s + 1:, :] = compute_prob(segs[i_s], segs[i_s + 1:], eps_segs[i_s], eps_segs[i_s + 1:],
                                                 dis_thres, angle_thres, eps_thres, ellp_fac=ellp_fac)
...
def update_prob_matrix(prob_ma, ind_connect, segs_up, eps_segs_up, dis_thres, angle_thres, eps_thres, ellp_fac=1):
    ...
    i_s = ind_connect[0]
    prob_ma[i_s, i_s + 1:, :] = compute_prob(segs_up[i_s], segs_up[i_s + 1:], eps_segs_up[i_s], eps_segs_up[i_s + 1:],
                                             dis_thres, angle_thres, eps_thres, ellp_fac=ellp_fac)
    prob_ma[:i_s, i_s, :] = compute_prob(segs_up[i_s], segs_up[:i_s], eps_segs_up[i_s], eps_segs_up[:i_s], dis_thres,
                                         angle_thres, eps_thres, ellp_fac=ellp_fac)[:, [0, 2, 1]]
...
def seg_reconnection(seg, segs, eps_segs, num_points_segs, dis_thres, angle_thres, eps_thres, ellp_fac=1):
    ...
    while num_pos_reconnect >= 1:
        ...
        # 2.b. Update segments
        seg, segs, eps_segs, num_points_segs = update_segs(ind_connect, ori_connect, seg, segs, eps_segs,
                                                           num_points_segs)

        # 2.c. Update probability matrix
        prob_ma = update_prob_matrix(prob_ma, ind_connect, segs, eps_segs, dis_thres, angle_thres, eps_thres,
                                     ellp_fac=ellp_fac)
...
def detect(im: np.array, th_dog=.1):
    # he = hist_eq(np.log(im))
    he = np.log(im)
    edges = DoG_leads(he, 6, 1.2)  # max_kern=5, min_kern=1

    leads = edges > th_dog  # threshold = 15
    lkf_thin = skimage.morphology.skeletonize(leads).astype('float')
    ...
    eps_mn = compute_mn_eps(np.log10(im), seg)
    ...
    # Initialize array containing start and end point of segments
    segs = np.array([np.stack([i[:, 0], i[:, -1]]).T for i in seg])
    seg = seg_reconnection(seg, segs, eps_mn, num_points_segs, dis_thres := 4,
                           angle_thres := 35, eps_thres, ellp_fac=(ellp_fac := 3))
    # Filter too short segments
    seg = filter_segs_lmin(seg, lmin := 4.8)
...
if __name__ == "__main__":
    ...
    bench = FourierData("C:/Users/Brend/PycharmProjects/MasterThesis/data/bench", dev=dev, phys_i=10,
                        scaling={'data': (0., torch.tensor([[[[0.0004184620047453791]], [[0.0003502570034470409]]]])),
                                 ...
                                 'A': (0., torch.tensor([[[[1.]]]]))})
    model = load_model('U MSRE bo/best', 0)
    out, H, A = propagate(model, bench, bench.data[0], bench.H[0], bench.A[0], bench.v_a, bench.v_o)
    sh_l = shear(strain_rate(bench.velocities))
    sh = shear(strain_rate(out))

    seg = detect(sh[-1].numpy())
    seg_l = detect(sh_l[-1].numpy())
